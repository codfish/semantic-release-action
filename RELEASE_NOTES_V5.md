...
- **npm OIDC Trusted Publishing Support**: The upgrade to @semantic-release/npm v13 enables support for npm's new
  OIDC-based trusted publishing. This allows publishing to npm without long-lived access tokens by using GitHub's OIDC
  token provider. This is more secure and eliminates the need to store `NPM_TOKEN` as a repository secret when
  publishing from GitHub Actions. See [npm documentation](https://docs.npmjs.com/generating-provenance-statements) for
...
- **npm OIDC Trusted Publishing**: With @semantic-release/npm v13, you can now use npm's OIDC-based trusted publishing
  instead of long-lived `NPM_TOKEN` secrets. This provides better security by using GitHub's OIDC token provider to
  authenticate npm publishes directly from GitHub Actions. No more storing sensitive npm tokens in repository secrets!

- **Automated major version tag updates**: The release workflow now automatically updates the major version tag (v5) to
  point to the latest release. Users binding to `@v5` will automatically receive the latest stable v5.x.x release.
